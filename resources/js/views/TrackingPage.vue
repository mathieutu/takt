...
const props = withDefaults(defineProps<{
    ...
    csrfToken?: string
    billingStoreAction?: string
    billingBaseAction?: string
    projectBudgetBase?: string
}>(), {
    clients:            () => [],
    selectedClientId:   null,
    projects:           () => [],
    csrfToken:          '',
    billingStoreAction: '/dashboard/tracking/billing',
    billingBaseAction:  '/dashboard/tracking/billing',
    projectBudgetBase:  '/dashboard/tracking/projects',
})
...
async function saveBilling() {
    ...
    const url   = isNew
        ? props.billingStoreAction
        : `${props.billingBaseAction}/${month.billing_entry_id}`
    ...
    try {
        const res = await fetch(url, {
            ...
            body: JSON.stringify({
                ...
                month:         month.month,
                amount_billed: parseFloat(billingForm.value.amount_billed) || 0,
                payment_date:  billingForm.value.payment_date || null,
                notes:         billingForm.value.notes || null,
            }),
        })
...
async function saveBudget() {
    ...
    const newBudget = value !== '' ? parseFloat(value) : null

    try {
        const res = await fetch(`${props.projectBudgetBase}/${project.id}/max-budget`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                'Accept':       'application/json',
            ...
            body: JSON.stringify({ max_budget: newBudget }),
        })
        if (!res.ok) throw new Error(`HTTP ${res.status}`)
        const data = await res.json()
            ...
            <div v-if="editingBilling" class="space-y-4">
                ...
                <div class="flex justify-end gap-2 pt-2">
                    ...
                    <button
                        ...
                        @click="saveBilling"
                    >{{ billingLoading ? 'Enregistrement…' : 'Enregistrer' }}</button>
                </div>
            </div>
        </Dialog>
            ...
            <div v-if="editingBudget" class="space-y-4">
                ...
                <div class="flex justify-end gap-2 pt-2">
                    ...
                    <button
                        ...
                        class="h-9 rounded-md bg-primary px-4 text-sm font-medium text-primary-foreground transition-colors hover:bg-primary/90 disabled:opacity-50"
                        @click="saveBudget"
                    >{{ budgetLoading ? 'Enregistrement…' : 'Enregistrer' }}</button>
                </div>
            </div>
        </Dialog>
    </div>
