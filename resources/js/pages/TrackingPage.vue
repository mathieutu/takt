...
async function saveBilling() {
  ...
  billingHttp.amount_billed = Number.parseFloat(billingForm.value.amount_billed) || 0
...
async function saveBudget() {
  ...
  budgetHttp.max_budget = value !== '' ? Number.parseFloat(value) : null
...
function formatMonth(ym: string): string {
  ...
  return `${MONTHS_FR[Number.parseInt(month, 10) - 1]} ${year}`
}
      ...
      <div class="mx-auto max-w-5xl space-y-6">
        <div>
          ...
          <p class="text-sm text-muted">
            Sélectionnez un client pour voir le détail de facturation par
            projet.
          </p>
        </div>
            ...
            <div class="flex items-center justify-between border-b border-default px-5 py-4">
              <div>
                ...
                <p class="text-xs text-muted">
                  TJ : {{ project.daily_rate.toLocaleString('fr-FR') }}
                  €/j
                </p>
              </div>
                ...
                <tbody>
                  ...
                  >
                    ...
                    <td class="px-4 py-2.5 text-right">
                      {{
                        fmt(month.days_worked * project.daily_rate)
                      }}
                    </td>
                    <td
              ...
              <div class="grid grid-cols-2 gap-x-8 gap-y-3 sm:grid-cols-4">
                <div>
                  ...
                  <p class="text-sm font-semibold">
                    {{
                      fmtDays(projectSummary(project).totalDays)
                    }}
                  </p>
                </div>
                <div>
                  ...
                  <p class="text-sm font-semibold">
                    {{
                      fmt(projectSummary(project).totalTheorique)
                    }}
                  </p>
                </div>
                <div>
                  ...
                  <p class="text-sm font-semibold text-emerald-600">
                    {{ fmt(projectSummary(project).totalEffectif) }}
                  </p>
                </div>
                <div>
                  <p class="text-xs text-muted">Reste à facturer</p>
                  <p class="text-sm font-semibold text-amber-600">
                    {{ fmt(projectSummary(project).resteAFacturer) }}
                  </p>
                </div>
                <template v-if="projectSummary(project).maxBudget !== null">
                  <div>
                    ...
                    <p class="text-sm font-semibold">
                      {{
                        fmt(projectSummary(project).maxBudget!)
                      }}
                    </p>
                  </div>
                  <div>
