...
const viewingTitle = computed(() => {
    ...
    const month = MONTHS_FR[parseInt(viewingReport.value.start_date.slice(5, 7)) - 1]
    return `${day} ${month}`
})
        ...
        <div class="flex h-[calc(100vh-3.5rem)] flex-col overflow-hidden px-3 py-4 md:px-6 md:py-6">
            <div class="mb-4 flex items-center justify-between">
                ...
                <div class="flex shrink-0 items-center gap-2">
                    ...
                    <span class="min-w-35 text-center text-sm font-medium text-default">{{ MONTHS_FR[displayMonth - 1] }} {{ displayYear }}</span>
            ...
            <div class="overflow-y-auto rounded-md border border-default max-h-full">
                <table class="border-collapse" style="table-layout: fixed; width: max-content; min-width: 100%;">
                    ...
                    <thead>
                        <tr>
                            <th class="sticky left-0 top-0 z-30 border-b border-r border-default bg-default px-3 py-2 text-left text-xs font-medium text-muted">Projet</th>
                            <th v-for="day in daysInMonth" :key="day.d"
                                ...
                                <div class="text-xs font-semibold leading-none" :class="day.dateStr === todayStr ? 'text-primary' : 'text-default'">{{ day.d }}</div>
                                <div class="mt-0.5 text-[10px] leading-none" :class="day.dateStr === todayStr ? 'text-primary' : 'text-muted'">{{ day.letter }}</div>
                            </th>
                        </tr>
                    </thead>
                    <tbody>
            ...
            <div class="mt-auto shrink-0 pt-3 flex items-center justify-between">
                <div class="hidden items-center gap-5 sm:flex">
                    <div class="flex items-center gap-1.5"><span class="h-3 w-3 rounded-sm border border-default bg-primary/25"></span><span class="text-xs text-muted">1 jour</span></div>
                    <div class="flex items-center gap-1.5"><span class="h-3 w-3 rounded-sm border border-default bg-primary/10"></span><span class="text-xs text-muted">½ jour</span></div>
                    <div class="flex items-center gap-1.5"><span class="h-3 w-3 rounded-sm border border-default bg-elevated"></span><span class="text-xs text-muted">Week-end</span></div>
                    <div class="flex items-center gap-1.5"><span class="h-3 w-3 rounded-sm border border-default bg-elevated"></span><span class="text-xs text-muted">Jour férié</span></div>
                </div>
    ...
    <div v-else class="flex h-screen flex-col overflow-hidden bg-default">
        ...
        <div class="flex flex-1 overflow-hidden">
            <main class="flex flex-1 flex-col overflow-hidden px-3 py-4 md:px-6 md:py-6">
                <div class="mb-4 flex items-center justify-between">
                    <h2 class="text-sm font-semibold text-default">{{ MONTHS_FR[displayMonth - 1] }} {{ displayYear }}</h2>
                    <div class="flex shrink-0 items-center gap-2">
                        <UButton icon="i-lucide-chevron-left" color="neutral" variant="ghost" size="xs" @click="prevMonth" />
                        <UButton icon="i-lucide-chevron-right" color="neutral" variant="ghost" size="xs" @click="nextMonth" />
                    </div>
